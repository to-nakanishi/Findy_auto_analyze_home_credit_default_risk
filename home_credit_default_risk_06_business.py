...
　ベースライン（元データ：申請を承認し、融資を行ったデータ）を一次審査とし、
  1次審査通過者に対して、当該モデルの正常Precision 95%ライン（閾値≒0.24）をスクリーニング試験運用を暫定推奨。
...
=================================================================
補足：
  - 暫定の推奨閾値T = 0.24（正常Precision ≥ 95%ライン）
...
#5-1　暫定の閾値推奨と根拠
'''
■ 暫定の推奨閾値：0.24前後（正常Precision ≥ 95%ライン）
...
# --- 1. 詳細データの計算 ---
fine_thresholds = np.arange(0.01, 0.36, 0.01)
...
# --- 2. デフォルトPrecisionの計算（大文字のTP, FPを使用） ---
# TP（正しく検知したデフォルト） / (TP + FP（モデルがデフォルトと予測した全件）)
fine_df['default_precision'] = fine_df['TP'] / (fine_df['TP'] + fine_df['FP'])

# --- 3. テーブル用データの作成 ---
# コスト内訳を消し、精度とビジネス指標に絞る
fine_display = fine_df[['threshold', 'normal_precision', 'default_precision', 'auto_approve_rate', 'improvement']].copy()
fine_display.columns = ['閾値', '正常Precision', 'デフォルトPrecision', '承認率(%)', '純改善額']

# --- 4. フォーマット整形 ---
fine_display['正常Precision'] = fine_display['正常Precision'].apply(lambda x: f"{x:.2%}")
fine_display['デフォルトPrecision'] = fine_display['デフォルトPrecision'].apply(lambda x: f"{x:.2%}")
fine_display['承認率(%)'] = fine_display['承認率(%)'].apply(lambda x: f"{x:.1f}%")
fine_display['純改善額'] = fine_display['純改善額'].apply(lambda x: f"{x:,.0f}")
...
#5-3　閾値推奨　推奨閾値付近の詳細比較　可視化

# 1. データの抽出（0.23に最も近い行を特定）
target_t = 0.23
idx_near = (fine_df['threshold'] - target_t).abs().idxmin()
best_row = fine_df.loc[idx_near]

# グラフ用の値を準備
x = fine_df['threshold']
y_prec_norm = fine_df['normal_precision']
y_improvement_ratio = (fine_df['improvement'] / fine_df['baseline_net'].iloc[0]) * 100
y_approve = fine_df['auto_approve_rate']

fig, ax1 = plt.subplots(figsize=(12, 7))
plt.subplots_adjust(right=0.85)

# --- 第1軸 (左): 正常Precision (予測の信頼性) ---
color_prec = '#1D9E75' # 緑
ax1.set_xlabel('閾値 (Threshold)', fontsize=11)
ax1.set_ylabel('正常Precision (返済予測の正確さ)', color=color_prec, fontsize=12, fontweight='bold')
line1 = ax1.plot(x, y_prec_norm, marker='o', color=color_prec, label='正常Precision', linewidth=2)
ax1.tick_params(axis='y', labelcolor=color_prec)
# 動きを強調するために上下限をデータに合わせる
ax1.set_ylim(y_prec_norm.min() - 0.005, y_prec_norm.max() + 0.005)
ax1.grid(True, alpha=0.2)

# --- 第2軸 (右1): 純利益改善率 (ビジネス成果) ---
ax2 = ax1.twinx()
color_ratio = '#0072B2' # 青
ax2.set_ylabel('純利益改善率 (%)', color=color_ratio, fontsize=12, fontweight='bold')
line2 = ax2.plot(x, y_improvement_ratio, marker='s', color=color_ratio, label='利益改善率', linewidth=2)
ax2.tick_params(axis='y', labelcolor=color_ratio)

# --- 第3軸 (右2): 自動承認率 (現場効率) ---
ax3 = ax1.twinx()
ax3.spines['right'].set_position(('outward', 60))
color_app = '#CC79A7' # ピンク
ax3.set_ylabel('承認率 (%)', color=color_app, fontsize=12, fontweight='bold')
line3 = ax3.plot(x, y_approve, marker='^', color=color_app, label='承認率', linewidth=1.5, linestyle=':')
ax3.tick_params(axis='y', labelcolor=color_app)

# --- 1. ベースラインの横線と注釈 ---
# ax2（右側第1軸：利益改善率）に対して描画
ax2.axhline(y=0, color='#D85A30', linestyle='-', linewidth=1.5, alpha=0.6)

# ベースラインのすぐ上にテキストを配置
# x座標を0.01にすることで左端に寄せ、y座標を利益0%の少し上に調整
ax2.text(0.01, 0.5, 'ベースライン (モデルなし)', color='#D85A30',
         fontsize=9, fontweight='bold', va='bottom', ha='left')

# --- 2. y軸の表示範囲を制限（スケール崩れ防止） ---
# 閾値0付近の極端な値をカットし、主要な変化を見やすくズームします
ax1.set_ylim(0.93, 1.0)        # 左軸：正常Precision
ax2.set_ylim(-15, 30)         # 右軸1：純利益改善率
ax3.set_ylim(-5, 105)         # 右軸2：承認率

# --- 3. x軸の表示範囲を指定 ---
ax1.set_xlim(0.0, 0.36)

# --- 5. 損益分岐点（0.09地点）のアノテーション修正 ---
# 0.09の行をピンポイントで取得
row_09 = fine_df[fine_df['threshold'] == 0.09].iloc[0]

# 垂直線は「理論上の分岐点（be_t ≈ 0.090）」に引く
ax1.axvline(x=be_t, color='gray', linestyle=':', alpha=0.6, linewidth=1.5)

# アノテーション
ax1.annotate(f'【損益分岐付近（参考）】\n'
             f'閾値: 0.09\n'
             f'利益改善: -0.2%\n'  # 実測値に合わせる
             f'承認率: {row_09["auto_approve_rate"]:.1f}%\n'
             f'正常精度: {row_09["normal_precision"]:.2%}\n'
             f'（※貸倒検知精度: {row_09["default_precision"]:.1%}）',
            xy=(0.09, -0.2), # 0.09のプロット地点を指す
            xycoords=ax2.transData,
            # グラフと重ならないよう、右側かつ少し下に配置
            xytext=(0.12, 0.938),
...
# --- 4. 最適閾値の垂直線と詳細ボックス（以下、既存コードに接続） ---
ax1.axvline(x=target_t, color='red', linestyle='--', alpha=0.8, linewidth=2)

ax1.annotate(f'【推奨閾値 {target_t:.2f}】\n'
             f'利益改善: +{best_row["improvement"]/best_row["baseline_net"]*100:.1f}%\n'
             f'承認率: {best_row["auto_approve_rate"]:.1f}%\n'
             f'正常精度: {best_row["normal_precision"]:.2%}\n'
             f'（※貸倒検知精度: {best_row["default_precision"]:.1%}）',
            # xy（矢印の先）を利益改善率（青い線）のピークに設定
            xy=(target_t, y_improvement_ratio.loc[idx_near]),
            xycoords=ax2.transData,
            xytext=(target_t + 0.03, 0.97),
            textcoords=ax1.transData,
...
plt.title('Home Credit: 利益・精度・効率の最適バランスの特定', fontsize=15, pad=25)
...
*   最適な閾値、正常プレシジョン  
正常プレシジョン97.55%以上はデフォルトリスクは低いものの、利益としては現状の審査のみで融資を行うよりも悪化する傾向にあり。また、承認率も55.2%であり2次審査のコストが増加。  
最も改善度が高い箇所は閾値0.23、正常プレシジョン値95.15%の値であり、ベースライン(モデルなし)比+22.3%となった。   
この点については正常プレシジョンの要求を高めると承認件数が減少し、計算式上のTN（承認の利益）が小さくなるため、改善率は低下する。人間の2次審査でFP（誤検知）を救済すれば利益は回復しうるが、その比重が大きくなるほどモデル導入の意義は薄れる。   
